...
class DOE:
    def __init__(self):
        ...
        self.highest_level_reached = False
    ...
    def plan_next_experiment(self, trace, woehler_points, ki_predictions, t_enough):
        ...
        if len(s_a_failure) == 0:
            load_levels= self.generate_load_levels_helper(ki_predictions)
            print('load_levels_failure_1', load_levels)
            print('chosen_load_level_failure_1', load_levels[-1])
            return load_levels[-1]
        elif len(s_a_failure) == 1:
            load_levels= self.generate_load_levels_helper(ki_predictions)
            print('load_levels_failure_2', load_levels)
            print('chosen_load_level_failure_2', load_levels[-2])
...
<<<<<<< HEAD
            print('load_levels_runout', load_levels)
=======
            print('load_levels_runnout', load_levels)
>>>>>>> 36c96d2f8ebec4c704bfc86ba345290e8d25d5bf
            load_level = self.find_best_load_level(trace, curve, load_levels,
                                              t_enough)
                                              
            print('chosen_load_level_runnout', load_level)
            ...
            woehler_params['one_t_s'] = az.plots.plot_utils.calculate_point_estimate(
                'mode', trace['one_t_s'])
            load_levels = self.generate_load_levels(ki_predictions) #using ki-predictions appears to lead to better results than using woehler_params for small data points.
            print('load_levels from generate_load_levels', load_levels) 
            print('load_levels shape from generate_load_levels', load_levels.shape) 
            load_level = self.find_best_load_level(trace, curve, load_levels,
                                              t_enough)
            return load_level
    ...
    def find_best_load_level(self, trace, curve, load_levels, t_enough):
        ...
        #load_levels, all_nmses, samples, samples_loc, samples_load_level = \
        #    self.local_search(load_levels,woehler_params, all_nmses,
        #                      samples, trace, curve, samples_load_level)
         
        #print('load_levels after appending, local search to generate_load_levels ', load_levels) 
        #print('load_levels shape after appending, local search to generate_load_levels', load_levels.shape)
        
        load_levels_args = np.argsort(load_levels)
        load_levels = np.sort(load_levels)
        all_nmses = all_nmses[load_levels_args]
        
        # Index from local search was not considered
        max_index = np.argmax(all_nmses)
        
        #if the expected prece is needed it can be calulated as follows:
        prices = self.calculate_prices(samples_load_level, load_levels)
        
        return load_levels[max_index]
        
    def local_search(self,load_levels, woehler_params, all_nmses,samples,trace,curve, samples_load_level):
        '''
        ...
        for factor in [-1,1]:
            ...
            while run:
                ...
                distributions, samples_append, samples_load_level_append = \
                    self.generate_distributions_load_level(trace, curve,
                                                          load_level_append)
                
                
                samples_load_level[load_level_append[0]] = samples_load_level_append[load_level_append[0]]
        ...
        return load_levels, all_nmses, samples, samples_app, samples_load_level
    
    def find_load_level_staircase(self, last_load_level, last_outcome, t):
        '''
        This function plannes the next experiment using the staircase methode in LLF (DF)
    ...
    def calc_max_load_level(self,):    
           
        max_s_a_list = []       
        for i in range(500):
            rand_int = random.randint(0, len(self.trace['k'])-1)
            k = self.trace['k'][rand_int]
            s_d_50 = self.trace['s_d'][rand_int]
            n_e = self.trace['n_e'][rand_int]

            max_s_a_list.append(10**(np.log10(s_d_50)-(1/k)*(np.log10(10e4)-np.log10(n_e))))
            
            
        mu = np.mean(max_s_a_list)
        sigma = np.std(max_s_a_list)
        
        upper_limit = self.max_s_a_orig + 15
        lower_limit = self.max_s_a_orig - 15
        
        potential_max_load_level = np.random.normal(mu, sigma)
        
        if lower_limit < potential_max_load_level < upper_limit:
            load_level_1 = self.max_s_a_orig
            self.highest_level_reached = True
        else:           
            highest_runout_orig = np.max(self.woehler_points_orig['s_a'][self.woehler_points_orig['outcome']=='runout'])
            zf_orig = self.woehler_points_orig['s_a'][self.woehler_points_orig['s_a']>highest_runout_orig]
            stepsize_zf = np.mean(np.diff(np.unique(zf_orig)))
            s_a_max_woehler_points = np.max(self.woehler_points['s_a'])
            
            if s_a_max_woehler_points + stepsize_zf > self.max_s_a_orig:
                load_level_1 = self.max_s_a_orig
                self.highest_level_reached = True
            else:
                load_level_1 = s_a_max_woehler_points + stepsize_zf
                
        return load_level_1
        
        
    def calc_max_load_level_simple(self,):    
    ...
    def generate_load_levels_helper(self,):
        '''
        ...
        if self.highest_level_reached == False:
            #load_level_1 = self.calc_max_load_level()
            load_level_1 = self.calc_max_load_level_simple()
        else:
            load_level_1 = self.max_s_a_orig
        
        #load_level_1 = load_level_3 + 9 / 10 * (load_level_1-load_level_3)

        #load_level_2 = np.max([(load_level_3 + 1 / 10 * (load_level_1-load_level_3))-load_level_3, load_level_3 + 10])
        
        load_level_2 = help_functions.calc_s_long(self.woehler_params, 0.95)
        load_level_3 = help_functions.calc_s_long(self.woehler_params, 0.7)
        load_level_4 = help_functions.calc_s_long(self.woehler_params, 0.3)
        
        load_levels = [load_level_1,load_level_2,load_level_3,load_level_4]
        
        return np.array(sorted(load_levels))
        
    def generate_load_levels(self, trace, woehler_params, woehler_points, woehler_points_orig, total_kerne=12, llf_kerne=8):
        '''
        ...
        '''
        self.trace = trace
        self.woehler_params = woehler_params
        self.woehler_points = woehler_points
        self.woehler_points_orig = woehler_points_orig
        self.max_s_a_orig = np.max(self.woehler_points_orig['s_a'])
              
        
        load_levels = self.generate_load_levels_helper()
        zf_kerne = total_kerne - llf_kerne
        
<<<<<<< HEAD
        zf_load_level = np.linspace(load_levels[0], load_levels[1]-10, zf_kerne)
        llf_load_level = np.linspace(load_levels[2]+10, load_levels[3], llf_kerne)
=======
        llf_load_level = np.linspace(load_levels[0]-10, load_levels[2], llf_kerne)
        zf_load_level = np.linspace(load_levels[2]+10, load_levels[3], zf_kerne)
>>>>>>> 36c96d2f8ebec4c704bfc86ba345290e8d25d5bf
        load_levels_2 = np.concatenate((llf_load_level, zf_load_level))
               
        
        return np.sort(np.unique(load_levels_2))
    ...
    def generate_distributions_load_level(self, trace, curve, load_levels):
        ...
        def parallelize_(trace_id, curve_id, load_level):
            #trace = ray.get(trace_id)
            #curve = ray.get(curve_id)
            trace = trace_id
            curve = curve_id
        ...
        def fast_calc(trace, curve, load_levels):
            print('Entered_fast_calc')
            trace_id = ray.put(trace)
            curve_id = ray.put(curve)

            result = ray.get([parallelize_.remote(trace_id, curve_id, load_level) for load_level in load_levels])             
            ...
            for i in range(len(result)):
                for key, value in result[i][2].items():
                    samples_load_level[key] = value
                    
            for i in range(len(result)):
                for params in ['k', 's_d', 'n_e', 'one_t_s']:
                    for key, value in result[i][0][params].items():
                        distributions[params][key] = value
                        
            for i in range(len(result)):
                for params in ['loads', 'cycles', 'outcome']:
                    samples[params] = np.append(samples[params], result[i][1][params])
            
            return distributions, samples, samples_load_level

        return fast_calc(trace, curve, load_levels)
    ...
    def calculate_nmses(self, distributions, woehler_params, load_levels):
        ...
        ----------
        distributions : Dict
            A dictionary containing samples from the distribition of the parameters, 
            that is expected after the next experiment.
        woehler_params : Dict
            A dictionary containing the four woehler parameter.
        load_levels : Array
            A one dimensional np array of the empircally determined,
